class IamUser
  attr_reader :id, :name, :mfa, :arn, :created, :last_login, :groups, :policies, :keys, :tags, :svc_acct, :email
  ...
  def last_login_days
    ...
    ( (Time.now-last_login) / 86400 ).to_i.to_s
  end
  ...
  def access_keys(iam_client)
    ...
    iam_client.list_access_keys({user_name: name}).access_key_metadata.map.each do |key|
      age_in_days = ((Time.now - key.create_date)/86400).to_i
      keys << {id: key.access_key_id, status: key.status, age_days: "#{age_in_days}"}
    end
    keys
  end
  ...
  def output_info(multiline=true)
    output = { Name: name, Groups: groups.join(' '), Email: email, MFA_enabled?: mfa, KeyAgeDays: key_age,
               LastLoginDays: last_login_days, Tags: tags_output }
